...
const ProductValidationSchema = z.object({
  name: z
    ...
    .min(5, { message: 'Must be 5 or more cherecters long' }),
  description: z
    .string({ required_error: 'Description is required' })
    .min(20, { message: 'Description minimum 20 Cherecter' })
    .max(150, { message: 'Description can be maximum 150 cherecter' }),
  price: z
    .number({ required_error: 'Price is required' })
    .positive('Price must be a positive number'),
  category: z
    .string({ required_error: 'Category is required' })
    .min(5, { message: 'Category is required' }),
